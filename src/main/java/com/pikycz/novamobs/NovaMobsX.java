...
public class NovaMobsX extends PluginBase implements Listener {
    ...
    public void onEnable() {
        ...
        if (AutoSpawnTask) {
            if (AutoSpawnTime > 0) {
                this.getServer().getScheduler().scheduleRepeatingTask(this, new AutoSpawnTask(this), AutoSpawnTime, true);
            }
        }
        ...
        for (Level level : Server.getInstance().getLevels().values()) {
            if (pluginConfig.getList("WorldsSpawnDisabled").contains(level.getFolderName().toLowerCase())) {
                continue;
            }
            levelsToSpawn.put(level.getId(), level);
        }

        if (!pluginConfig.get("ConfigVersion").equals("1.0")) {
            getLogger().info(PluginPrefix + TextFormat.RED + "Please remove NovaMobs file > config.yml");
        }

        //register Passive entities
        this.registerEntity(Bat.class.getSimpleName(), Bat.class);
                ...
                if (args.length == 0) {
                    ...
                    switch (args[0]) {
                        ...
                        case "removeitems":
                            ...
                            break;
                        case "version":
                            sender.sendMessage(PluginPrefix + TextFormat.GREEN + " Version " + IntVersion + " working with MCPE 1.2.13");
                            break;
                        default:
